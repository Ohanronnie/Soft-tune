...
function meta(){
 jsmediatags.read(event.target.files[0], {
  ...
  music_details = `data:${format};base64,${window.btoa(base64String)}[${tag.tags.title}[${tag.tags.album}[${tag.tags.artist}` 
...
form.addEventListener('submit',function(e){
	...
	let formdata = new FormData(form);
        formdata.append("metadata",music_details);
	fetch(port + "/upload",{
		method: "post",
		body: formdata
	});
...
/* async function lastplayed(){
  ...
  /* let req = JSON.parse(re);
   ...
   newdiv.innerHTML = `<img src="${reimg}" data-src="${rest.cover}" onclick="setStreams(this)"><p>${rest.title}</p><span>${rest.album}</span>`
...
//let now_playing = document.querySelector(".now-playing");
     let updateTimer;
     let track_index = 0;
     let track_art = document.querySelector(".img-cover-last");
     let track_name = document.querySelector(".song-last");
     let track_artist = document.querySelector(".title-cover-last");
