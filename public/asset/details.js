...
async function list(){
  try{
   let res = await fetch(port+ '/musicList');
   let re = await res.json();
  /* let req = JSON.parse(re);
   alert(req)
   */for(let resp of re){
   let img = await fetch(port + '/images/'+resp.cover);
   let img_cover = await fetch(port + '/images/'+re[re.length - 1].cover)
...
let locat = ['/music/'+file]; 
myff = (a,b,c,d) =>{
    let x;
    alert([a,b,c,d]);
      if([a,b,c,d] == '' || [a,b,c,d] == null){
      clearInterval(x)
      audio = audio;
      audio = new Audio(locat)
      alert(a,b,c,d)
    }
    else{
      clearInterval(x)
      locat.push('/music/'+d);
      alert(locat)
      audio = new Audio(locat[locat.length - 1]);
    }
    x = () => {
